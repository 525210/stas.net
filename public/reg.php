                        ...
                        <div class="col my-auto py-lg-5">
                            <div class="row bg-light rounded text-center mx-0 py-4 mb-4 mb-lg-0">
                                <div class="col mx-auto my-3">
                                    <p class="text-black-50 mb-4">רישום למערכת</p>
                                    <!-- Contact Form -->
                                    <form action="" method="post">
                                        <div class="position-relative icon-group mb-3 text-right">
                                            <input name="name" type="text" class="form-control border-2" placeholder="שם מלא">
                                        </div>
                                        <div class="position-relative icon-group mb-3 text-right">
                                            <input name="email" type="email" class="form-control border-2" placeholder="אימייל">
                                        </div>
                                        <div class="position-relative icon-group mb-3 text-right">
                                            <input name="password" type="password" class="form-control border-2" placeholder="סיסמה">
                                        </div>
                                        <div class="position-relative icon-group mb-3 text-right">
                                            <input name="re_password" type="password" class="form-control border-2" placeholder="אימות סיסמה">
                                        </div>
                                    ...
                                    if (!empty($_POST["name"]) || $_POST["email"] || $_POST["password"] || $_POST["re_password"])
                                    {

                                        $user = $html->cleanInputData($_POST["name"]);
                                        $email = $html->cleanInputData($_POST["email"]);
                                        $password = $html->cleanInputData($_POST["password"]);
                                        $re_password = $html->cleanInputData($_POST["re_password"]);
                                        $html->register($user, $email, $password, $re_password);
                                    }
                                    ?>
                                    <div class="mt-4">
                                        <a href="log.php">התחברות</a>
